<?php
	...
	class Auth extends CI_Controller {
		...
		public function index(){
			...
			{
				redirect('dataTable/documentation?id_origen=1');
			}
			else{
		...
		public function login(){
			...
			if($this->input->post('submit')){
				...
				if ($this->form_validation->run() == FALSE) {
					...
					if ($data['captcha'] == $data['resultado']) {
						if ($result == TRUE) {
							$admin_data = array(
								'admin_id' => $result['id'],
						...
						if ($result['role'] == 1) {
							...
							redirect(base_url('dataTable/documentation?id_origen=1'), 'refresh');
						}else{
							$data['msg'] = 'Lutilisateur appartient à une autre application';
