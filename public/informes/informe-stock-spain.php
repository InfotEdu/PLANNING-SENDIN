...
$sqlHA ="SELECT
  t.nom_articulo,
  SUM(t.d6_spain)    AS d6_spain,
  MAX(t.peso_d6)     AS peso_d6,
  SUM(t.d8_spain)    AS d8_spain,
  MAX(t.peso_d8)     AS peso_d8,
  SUM(t.d9_spain)    AS d9_spain,
  MAX(t.peso_d9)     AS peso_d9,
  SUM(t.d10_spain)   AS d10_spain,
  MAX(t.peso_d10)    AS peso_d10,
  SUM(t.d12_spain)   AS d12_spain,
  MAX(t.peso_d12)    AS peso_d12,
  SUM(t.d14_spain)   AS d14_spain,
  MAX(t.peso_d14)    AS peso_d14,
  SUM(t.d16_spain)   AS d16_spain,
  MAX(t.peso_d16)    AS peso_d16,
  SUM(t.d20_spain)   AS d20_spain,
  MAX(t.peso_d20)    AS peso_d20,
  SUM(t.d25_spain)   AS d25_spain,
  MAX(t.peso_d25)    AS peso_d25,
  SUM(t.d32_spain)   AS d32_spain,
  MAX(t.peso_d32)    AS peso_d32,
  SUM(t.d40_spain)   AS d40_spain,
  MAX(t.peso_d40)    AS peso_d40,
  SUM(t.d6_chilly)   AS d6_chilly,
  SUM(t.d8_chilly)   AS d8_chilly,
  SUM(t.d9_chilly)   AS d9_chilly,
  SUM(t.d10_chilly)  AS d10_chilly,
  SUM(t.d12_chilly)  AS d12_chilly,
  SUM(t.d14_chilly)  AS d14_chilly,
  SUM(t.d16_chilly)  AS d16_chilly,
  SUM(t.d20_chilly)  AS d20_chilly,
  SUM(t.d25_chilly)  AS d25_chilly,
  SUM(t.d32_chilly)  AS d32_chilly,
  SUM(t.d40_chilly)  AS d40_chilly
FROM (
  -- tu consulta original sin ORDER BY
  SELECT
    sm.nom_articulo,
    sm.d6_spain,    sad6.peso_paquete   AS peso_d6,
    sm.d8_spain,    sast20.peso_paquete AS peso_d8,
    sm.d9_spain,    sast25.peso_paquete AS peso_d9,
    sm.d10_spain,   sad10.peso_paquete  AS peso_d10,
    sm.d12_spain,   sad12.peso_paquete  AS peso_d12,
    sm.d14_spain,   sad14.peso_paquete  AS peso_d14,
    sm.d16_spain,   sast50.peso_paquete AS peso_d16,
    sm.d20_spain,   sad20.peso_paquete  AS peso_d20,
    sm.d25_spain,   sad25.peso_paquete  AS peso_d25,
    sm.d32_spain,   sad32.peso_paquete  AS peso_d32,
    sm.d40_spain,   sad40.peso_paquete  AS peso_d40,
    sm.d6_chilly,
    sm.d8_chilly,
    sm.d9_chilly,
    sm.d10_chilly,
    ...
    sm.d40_chilly,
    sm.orden
  FROM sen_stock_mat AS sm
    JOIN sen_stock_art AS sad6   ON sad6.id_articulo = sm.id_articulo AND sad6.formato   = 'd6_spain'
    JOIN sen_stock_art AS sast20 ON sast20.id_articulo = sm.id_articulo AND sast20.formato = 'd8_spain'
    JOIN sen_stock_art AS sast25 ON sast25.id_articulo = sm.id_articulo AND sast25.formato = 'd9_spain'
    JOIN sen_stock_art AS sad10  ON sad10.id_articulo = sm.id_articulo AND sad10.formato  = 'd10_spain'
    JOIN sen_stock_art AS sad12  ON sad12.id_articulo = sm.id_articulo AND sad12.formato  = 'd12_spain'
    JOIN sen_stock_art AS sad14  ON sad14.id_articulo = sm.id_articulo AND sad14.formato  = 'd14_spain'
    JOIN sen_stock_art AS sast50 ON sast50.id_articulo = sm.id_articulo AND sast50.formato = 'd16_spain'
    JOIN sen_stock_art AS sad20  ON sad20.id_articulo = sm.id_articulo AND sad20.formato  = 'd20_spain'
    JOIN sen_stock_art AS sad25  ON sad25.id_articulo = sm.id_articulo AND sad25.formato  = 'd25_spain'
    JOIN sen_stock_art AS sad32  ON sad32.id_articulo = sm.id_articulo AND sad32.formato  = 'd32_spain'
    JOIN sen_stock_art AS sad40  ON sad40.id_articulo = sm.id_articulo AND sad40.formato  = 'd40_spain'
  WHERE
    sm.fecha   = '".$fecha."'    AND sm.familia = 1
         ) AS t
         GROUP BY
         t.nom_articulo
         ORDER BY
