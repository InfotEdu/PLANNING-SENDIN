...
$(document).ready(function() {
	...
	var editor = new $.fn.dataTable.Editor( {
		ajax: 'php/table.cartasadmin.php?id='+sessionValue,
		table: '#cartas',
		fields: [
			{
				"label": "Id Cliente:",
	...
	editor.on( 'submitComplete', function ( e, json, data, action) {
		...
		if (action == 'edit') {
		  $.getJSON( 'https://tucartadigital.app/public/appService/generaBlobComunidad.php/?id_file='+data.cartas.id_file+'&id_carta='+data.DT_RowId+'&id_cliente='+data.cartas.id_cliente, function( data2 ) {
	...
	var table = $('#cartas').DataTable( {
		...
		columns: [
			...
			{ data: null, render: function ( data, type, row ) {
                return '<a href="https://tucartadigital.app/upload/'+data.cartas.id_cliente+'/'+data.cartas.id_carta+'.pdf" target="_blank" />Ver carta</a>';
            } },
            { data: null, render: function ( data, type, row ) {
