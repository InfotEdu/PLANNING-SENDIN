...
$(document).ready(function() {
    ...
    if (editable == 1) {
        var editor = new $.fn.dataTable.Editor( {
            ajax: 'php/table.stock-mat-aboutvoile-chilly.php/?fecha='+fecha,
            table: '#ABOUTVOILE',
            fields: [
                {
        ...
        $('#ABOUTVOILE').on( 'click', 'tbody td:not(:first-child)', function (e) {
            editor.inline( this );
        } );
...
// Setup - add a text input to each footer cell
    $('#ABOUTVOILE tfoot th').each( function () {
        var title = $(this).text();
        $(this).html( '<input type="text" placeholder="Chercher '+title+'" />' );
    } );

	var table = $('#ABOUTVOILE').DataTable( {
		ajax: 'php/table.stock-mat-aboutvoile-chilly.php/?fecha='+fecha,
		columns: [
            {
			...
			{
				"data": "sen_stock_mat.sec_a4_chilly"
			},
			{
				"data": "sen_stock_mat.poutre_ha8_chilly"
			},
			{
				"data": "sen_stock_mat.poutre_ha10_chilly"
			}
		],
		paging: false,
		searching: false,
